...
schedule.every().day.at('17:07').do(WeekPrevsThread, db)    # 22:00
schedule.every().day.at('17:05').do(todayDocumentThread, db, api_key)   # 00:01
schedule.every().hour.at(':06').do(insertHourlyThread, db, api_key)     # :02
schedule.every().day.at('17:08').do(calculateAccuracy, db, "3bmeteo")   # 23:30
schedule.every().day.at('17:08').do(calculateAccuracy, db, "meteoit")   # 23:30
schedule.every().day.at('17:09').do(findNewNames, db)   # 23:40
